...
export const toolInputSchemas = {
  ...
  searchWeb: validator((input) => {
    ...
    return {
      ...
      value: {
        query: input.query,
        count: typeof input.count === "number" && input.count >= 1 && input.count <= 20 ? input.count : 5,
      },
    };
  }),
  batchEdit: validator((input) => {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      if (typeof op.filePath !== "string") return { ok: false, error: `operations[${i}].filePath is required` };
      if (typeof op.oldString !== "string") return { ok: false, error: `operations[${i}].oldString is required` };
      if (typeof op.newString !== "string") return { ok: false, error: `operations[${i}].newString is required` };
    }
    return {
      ok: true,
      value: {
