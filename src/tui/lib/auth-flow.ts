...
export function getCurrentProvider(): AuthProvider | null {
  if (!active || currentIndex < 0 || currentIndex >= providers.length) return null;
...
export function startAuthWizard(): string {
  active = true;
  currentIndex = 0;
  providers = [];
...
export async function initializeProviders(): Promise<string> {
  ...
  const defined: any[] = config.get('ai.providers', []);
  const missing: AuthProvider[] = [];

  for (const p of defined) {
    if (p.provider === 'local' || p.provider === 'ollama') continue;
...
export async function handleAuthInput(key: string): Promise<string> {
