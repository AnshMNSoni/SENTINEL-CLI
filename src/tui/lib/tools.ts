...
export const TOOLS: Record<string, ToolDefinition> = {
  ...
  analyze: {
    ...
    readOnly: true,
    execute: async ({ files, mode: m }: Record<string, unknown>) => runAnalysis(String(files || ".")),
  },
  securityAudit: {
    name: "securityAudit",
  ...
  scanSecrets: {
    ...
    readOnly: true,
    execute: async ({ target }: Record<string, unknown>) => runAnalysis(String(target || "."), { secrets: true }),
  },
  diff: {
    name: "diff",
    ...
    execute: async () => {
      try {
        const out = execSync("git diff --staged", { encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
  ...
  status: {
    ...
    execute: async () => {
      ...
      } catch { return { success: true, output: "Sentinel v1.9.0" }; }
    },
  },
  stats: {
    name: "stats",
    ...
    execute: async () => {
      try {
        ...
        return { success: true, output: `Dependencies: ${deps} prod, ${devDeps} dev\nScripts: ${Object.keys(pkg.scripts || {}).length}` };
      } catch { return { success: true, output: "No package.json found" }; }
    },
  },
  fix: {
    name: "fix",
    ...
    execute: async ({ files }: Record<string, unknown>) => {
      ...
      try {
        ...
        if (issues && Array.isArray(issues) && issues.length > 0) {
          ...
          await bot.shutdown();
          return { success: true, output: `Fixed ${issues.length} issues: ${JSON.stringify(result)}` };
        }
        await bot.shutdown();
  ...
  agents: {
    ...
    readOnly: false,
    execute: async ({ input }: Record<string, unknown>) => runAnalysis(".", { agent: true, input: String(input || "") }),
  },
  preCommit: {
    name: "preCommit",
...
export function getToolsForMode(mode: "BUILD" | "PLAN" | "SCAN" | "FIX"): Record<string, ToolDefinition> {
  const all = { ...TOOLS };
  if (mode === "PLAN") {
  ...
  if (mode === "FIX") {
    const filtered: Record<string, ToolDefinition> = {};
    for (const [key, tool] of Object.entries(all)) {
      if (!tool.readOnly || key === "readFile" || key === "glob" || key === "grep" || key === "listDirectory" || key === "diff") {
        filtered[key] = tool;
      }
    }
    return filtered;
