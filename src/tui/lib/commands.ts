...
export const COMMAND_HANDLERS: Record<string, CommandHandler> = {
  ...
  auth: {
    ...
    handler: async (args: string) => {
      ...
      if (subcmd === 'remove') {
        if (!parts[1]) return 'Usage: /auth remove <provider>';
        const provider = parts[1].toLowerCase();
        try {
  ...
  ollama: {
    ...
    handler: async (args: string) => {
      ...
      if (!trimmed) {
        ...
        if (models.length === 0) return 'No Ollama models found. Is Ollama running?';
      ...
      if (!match) return 'Usage: /ollama <model> <prompt>';
      const model = match[1];
      ...
      try {
        ...
        if (!res.ok) return `Ollama error (${res.status}): ${res.statusText}`;
  ...
  commit: {
    ...
    handler: async () => {
      try {
        ...
        if (!diff.trim()) return 'No staged changes. Stage files with `git add` first.';
        const msg = await chat(
          `Generate a concise conventional commit message (subject ≤72 chars + body if needed) for this diff:\n\n${diff.slice(0, 4000)}`
        );
    ...
    handler: async (args: string) => {
      ...
      try {
        const result = await chat(
          `Answer this question using your knowledge. If you're unsure, say so. Query: ${args}`,
          { systemPrompt: 'You are a helpful search assistant. Be concise and cite facts.' }
        );
        return result || '(no response)';
      } catch (e) {
        return `Error: ${e}`;
  ...
  trends: {
    ...
    handler: async () => {
      ...
      if (!existsSync(sessDir)) return 'No session data for trend analysis.';
      try {
        const files = readdirSync(sessDir)
          ...
          .reverse()
          .slice(0, 14);
        if (files.length === 0) return 'No session data for trend analysis.';
  ...
  'secret-patterns': {
    ...
    handler: async () => {
      try {
        ...
        if (patterns.length === 0) return 'Secret patterns: (none loaded)';
  ...
  sessions: {
    ...
    handler: async (args: string) => {
      ...
      const home = process.env.HOME || process.env.USERPROFILE || '~';
      const sessDir = `${home}/.sentinel/sessions`;
      if (!existsSync(sessDir)) return 'No saved sessions found.';
      try {
        const files = readdirSync(sessDir)
          .filter(f => f.endsWith('.json'))
          .sort()
          .reverse();
        if (files.length === 0) return 'No saved sessions found.';
        if (trimmed) {
          const found = files.find(f => f.toLowerCase().includes(trimmed));
          if (!found) return `No session matching "${trimmed}".`;
  ...
  validate: {
    ...
    handler: async () => {
      ...
      try {
        ...
        if (!existsSync(configPath)) return 'No .codereviewrc.json found.';
        ...
        for (const p of cfg.ai?.providers || []) {
          if (!p.provider) issues.push(`Provider missing name: ${JSON.stringify(p)}`);
          if (p.enabled !== false && !p.apiKeyEnv && p.provider !== 'ollama') {
            issues.push(`${p.provider}: no apiKeyEnv set`);
          }
        }
        if (!cfg.analysis?.enabledAnalyzers) issues.push('Missing analysis.enabledAnalyzers');
        const home = process.env.HOME || process.env.USERPROFILE || '~';
        const prefsPath = `${home}/.sentinel/preferences.json`;
  ...
  'test-suggestions': {
    ...
    handler: async (args: string) => {
      ...
      try {
        ...
        if (diff.trim()) {
          const result = await chat(
            `Review this diff and suggest unit tests (file: ${target}):\n\n${diff.slice(0, 3000)}`,
      ...
      const result = await chat(
        `Suggest unit tests for the codebase at ${target}. Focus on the main entry points.`,
        { systemPrompt: 'You are a test expert. Suggest specific test cases with frameworks.' }
      );
      return result || '(no response)';
    },
  ...
  'pr-description': {
    ...
    handler: async () => {
      try {
        ...
        const msg = await chat(
          `Generate a concise PR description based on:\n\nRecent commits:\n${diff}\n\nChanged files:\n${files}`,
          { systemPrompt: 'You are a technical writer. Generate clear PR descriptions.' }
        );
        return msg || '(no response)';
      } catch (e) {
        return `Error: ${e}`;
  ...
  'pr-summary': {
    ...
    handler: async () => {
      try {
        ...
        const msg = await chat(
          `Summarize this PR diff concisely. Include: what changed, why, any risks:\n\n${diff.slice(0, 4000)}`,
          { systemPrompt: 'You are a code reviewer. Summarize PRs clearly.' }
        );
        return msg || '(no response)';
      } catch (e) {
        return `Error: ${e}`;
