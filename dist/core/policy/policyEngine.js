...
class PolicyEngine {
  ...
  async loadPolicies(policyPaths = []) {
    ...
    for (const policyPath of paths) {
      try {
        const s = await stat(policyPath);
        if (s.isDirectory()) {
          const files = await readdir(policyPath);
          for (const file of files) {
            if (file.endsWith('.json') || file.endsWith('.yaml') || file.endsWith('.yml') || file.endsWith('.js')) {
              await this.loadPolicyFromPath(path.join(policyPath, file));
            }
          }
        } else {
