  ...
  if (issues.length === 0) {
    ...
    for (const [sev, list] of Object.entries(bySeverity)) {
      ...
      for (const issue of list.slice(0, 20)) {
        // cap at 20 per severity
        lines.push(`• [${issue.ruleId ?? issue.id ?? '?'}] ${issue.message ?? issue.description}`);
        if (issue.file) {
...
server.tool(
  ...
  {
    target: z.string().describe("File path or directory to audit. Example: 'src/', '.'"),
    includeSecrets: z
      .boolean()
      .optional()
      .default(true)
  ...
  async ({ target, includeSecrets, includeDeps }) => {
    ...
    const result = await runSentinel(['analyze', target, '--analyzers', analyzers.join(',')]);
    return { content: [{ type: 'text', text: formatResult(result) }] };
  }
);
...
server.tool(
  ...
  'Analyze a code snippet passed as a string — no file needed. Ideal when the user has just written or pasted code and wants immediate feedback. Returns issues with severity, line numbers, and fix suggestions.',
  {
    ...
    language: z
      .enum(['javascript', 'typescript', 'python', 'go', 'java', 'ruby', 'php', 'html', 'css'])
      .optional()
      .default('javascript')
...
server.tool(
  ...
  'Get a detailed explanation of a specific Sentinel issue — what the vulnerability is, how an attacker could exploit it, a fixed code example, and links to OWASP/CWE references. Use when the user wants to understand a finding, not just see it listed.',
  {
    ...
    codeContext: z
      .string()
      .optional()
      .describe('Optional: the actual vulnerable code snippet for a tailored explanation.'),
  },
...
server.tool(
  ...
  'Apply automatic fixes to common issues in a file or directory. Fixes console.log removal, missing alt attributes, simple security anti-patterns, and more. Use --dryRun to preview changes before applying.',
  {
    ...
    analyzers: z
      .string()
      .optional()
      .describe('Comma-separated analyzers whose issues should be fixed. Defaults to all fixable.'),
  },
...
server.tool(
  ...
  {
    directory: z.string().optional().default('.').describe('Project root directory to score.'),
  },
  async ({ directory }) => {
...
server.resource(
  ...
  async () => {
    try {
      const { stdout } = await execFileAsync('sentinel', ['list-analyzers', '--format', 'json']);
    ...
    try {
      ...
      return {
        contents: [
          {
            uri: 'sentinel://config',
            text: 'No .sentinel.json found. Launch the TUI (sentinel) and use /auth to configure, or create one manually. See https://github.com/KunjShah95/SENTINEL-CLI for the config schema.',
          },
        ],
      };
    }
